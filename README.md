...
Mutable arrays running in kernel on any devices.
...
which means a physical element has: mass $m$, density $\rho$, velocity vector $\vec{u}=(u,v)$ and stress tensor $\sigma=\begin{pmatrix}s_{11} & s_{12} \\ s_{21} & s_{22}\end{pmatrix}$.
...
`BabelArrays.jl` provides such a mutable container type `BArray` that can be used in kernel functions.
...
@kernel function f!(data)
    ...
    m = data[idx, nt.m]
    rho = data[idx, nt.rho]
    u = BVector{2}(idx, nt.u, data) # subtype of StaticVector
    sigma = BMatrix{2, 2}(idx, nt.sigma, data) # a subtype of StaticMatrix
    ... # operate the container as `StaticArrays.MArray`
end
```

Other memory layouts can be extended by implementing the `AbstractBabelArray` interface like `BArray <: AbstractBabelArray`.